...
| `src/gateways/audit-stream.ts` | Shared `streamToAuditFile`, `formatSyntheticEvent`, `appendToAuditFile`, and `generateTempFilename` used by all runners — writes each JSONL line as it arrives so coordinators can watch progress in real time; `generateTempFilename(prefix, ext)` produces a unique temp path under the OS temp dir |
