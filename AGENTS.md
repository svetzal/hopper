...
| `src/profile.ts` | `ModelTier` vocabulary (`deep`/`balanced`/`fast`), `isModelTier`, profile parsing (`parseProfile`, `resolveSessionBinding`), and required-key validation. The tier→model binding lives in each profile's `models` map; claude's tier→`opus\|sonnet\|haiku` translation is a per-profile concern |
| `src/gateways/routing-runner.ts` | Profile-driven router that wraps the claude, opencode, and codex runners; dispatches `AgentRunner` calls to the correct concrete runner based on `item.profile` |
| `src/gateways/runner-session.ts` | Shared `buildRunnerRunSession` template — per-runner hook object over `spawnStreamedSession`; used by the opencode and codex runners to avoid duplicating spawn/audit wiring |
| `src/gateways/claude-argv.ts` | Pure argv builder for `claude --print` invocations |
| `src/gateways/codex-argv.ts` | Pure argv builder for `codex exec --json` invocations |
| `src/gateways/agents-gateway.ts` | Thin I/O wrapper over `~/.claude/agents/` and `<project>/.claude/agents/`; reads `*.md` files and parses frontmatter via `craftsperson-resolver.ts` to produce `AgentCandidate` lists |
| `src/gateways/audit-gateway.ts` | Enumerates per-phase JSONL audit files (`PhaseFile`) under the audit directory for a given item |
| `src/gateways/json-file.ts` | Shared `loadJsonFile`/save helpers for `~/.hopper/*.json` files; used by store, preset, and profiles gateways |
| `src/gateways/profiles-gateway.ts` | I/O wrapper over `~/.hopper/profiles/<name>.json`; reads, writes, and lists profiles via `parseProfile` from `src/profile.ts` |
| `src/gateways/resolve-bin.ts` | `resolveBinOnPath(name, installHint)` — `Bun.which` lookup that throws a clear error with an install hint if the binary is absent from `PATH` |
...
| `src/audit-workflow.ts` | Pure audit decoding: `PhaseInput`, `DecodedEvent`, `AuditSummary` — transforms JSONL lines into structured audit reports |
| `src/engineering-errors.ts` | `EngineeringBranchConflictError` — typed error thrown by `orchestrateWorktreeSetup` when the computed work branch already exists and cannot be safely reclaimed |
| `src/error-utils.ts` | `toErrorMessage(e)` — normalises any thrown value to a string |
| `src/extract-cost.ts` | Pure per-phase token/cost telemetry extractor from JSONL audit streams for both the claude and opencode runners |
| `src/git-workflow.ts` | Pure branch-setup decision functions: `BranchSetupAction`, `BranchCheckResult` |
| `src/is-record.ts` | `isRecord` type guard — true when a value is a plain object (not null, not an array) |
| `src/parse-time.ts` | Pure `parseDuration(input)` and `parseTimeSpec(input, now)` with injected clock (no `Date.now()` calls) |
| `src/priority.ts` | `Priority` type, `parsePriority`, `priorityBadge`, and priority comparison helpers |
| `src/result.ts` | `Result<T,E>` discriminated union, `ok`/`err` constructors, `unwrapOrError` |
| `src/tags.ts` | `normalizeTag`, `mergeTags`, `matchesTags`, `tagBadge` — tag validation and manipulation |
| `src/task-type-workflow.ts` | Shared prompt-preamble builder, `INVESTIGATION_DISALLOWED_TOOLS` denylist, task-type → phase/model/tool configuration |
...
| `src/commands/add-agent-resolver.ts` | Craftsperson-resolution helper for the `add` command: reads candidates via `agents-gateway.ts`, builds a Haiku prompt, selects the best-fitting agent |
...
- **Investigation sandbox**: Investigation items run with the full `Bash` tool plus `INVESTIGATION_DISALLOWED_TOOLS` (in `src/task-type-workflow.ts`) which blocks git mutations, hopper queue mutations, foundry/evt mutators, package installers, most network-egress CLIs, and destructive filesystem verbs. **Read-only `aws` is an exception — allowed since 3.4.0** (see the shim note below), so an investigation can verify live production state directly instead of relying on inlined evidence. Brief authors can assume read access to `hopper show|list|audit|history`, `git log|status|diff|show|rev-parse|blame`, `git worktree list`, `jq`, `cat|ls|head|tail|wc|find|grep|rg|awk|sed`, `foundry history|trace`, `evt query|aggregate`, and read-only `aws` (`get-*`/`describe-*`/`list-*`/`query`/`scan`/`batch-get-item`, e.g. `aws dynamodb get-item`, `aws sts get-caller-identity`). Note: `Bash(git branch:*)` is denied (blocks `-d|-D` mutations but also `--list`); use `git for-each-ref refs/heads` instead. `Bash(git stash:*)` similarly blocks `stash list`; use `git log refs/stash`. `permissionMode: "plan"` is **not** applied — the sandbox uses a two-layer defence instead:
  ...
  2. **PATH shims in `~/.hopper/worker-shims/`** — close the shell-composition bypass. Hopper prepends this directory to `PATH` (via `HOPPER_REAL_PATH`/`PATH` env vars in `SessionOptions.env`) so `git`, `curl`, etc. resolve to tiny `/bin/sh` scripts that inspect `$1` and exit 1 with a clear deny message for forbidden verbs, regardless of how Bash composed the call. Most shims are deny-lists; the `aws` shim (since 3.4.0) is the inverse — an allow-list that scans past global flags to the service/action token and permits only read-only actions (`get-*`/`describe-*`/`list-*`/`query`/`scan`/`batch-get-item`), denying every mutation and any unrecognised action by default (`buildAwsReadonlyShimScript` / `AWS_READONLY` in `worker-shim-content.ts`, re-injected via `buildInvestigationShimMap`). Shims are regenerated idempotently at every `hopper worker` startup via `WorkerShimGateway.synchronize`. The shim layer also covers the opencode runner, which silently ignores `disallowedTools`. On Windows, PATH shims are POSIX-only; Windows investigation sessions fall back to the denylist guardrail alone.
...
- **Model tiers**: Hopper addresses models through a vendor-agnostic three-tier vocabulary (`deep`, `balanced`, `fast` — defined in `src/profile.ts`) chosen per phase in `src/task-type-workflow.ts`. Each runner resolves tiers through the per-item profile loaded by `src/gateways/profiles-gateway.ts`; the tier→model binding lives in the profile's `models` map. Runner-native strings (e.g. `openai/gpt-5.5`) pass through untranslated, so an advanced caller can still pin a specific model.
...
- **Error handling strategy**: Three distinct layers — never call `console.error` outside `command-runner.ts`:
  1. **Gateway layer** (`src/gateways/`): Two sub-contracts by gateway kind:
     - **Read/query gateways** (`StoreGateway`, `PresetGateway`, `LlmGateway`, read operations in `FsGateway` and `InitGateway`): Silent catch-and-return-empty/null. Missing files, unreadable directories, non-OK HTTP responses, and parse failures return neutral defaults (e.g. `[]`, `null`, `{ choices: [] }`). No logging. External data shapes are validated at trust boundaries before casting: the store uses `isValidItemShape`, `LlmGateway` uses `isLlmCompletionResponse` (guards `choices` is an array), and `scanOpencodeStream`/`parseOpencodeExport` in `extract-opencode-result.ts` guard `type` is a string and `messages` is an array (when present) before narrowing to typed shapes.
